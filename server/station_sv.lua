if Config.PlayerOwnedGasStationsEnabled then -- This is so Player Owned Gas Stations are a Config Option, instead of forced. Set this option in shared/config.lua!
    ...
    RegisterNetEvent('cdn-fuel:station:server:Withdraw', function(amount, location, StationBalance)
        ...
        if Config.FuelDebug then print("Attempting to withdraw 𝕍"..amount.." from Location #"..location.."'s Balance!") end
    ...
    RegisterNetEvent('cdn-fuel:station:server:Deposit', function(amount, location, StationBalance)
        ...
        local Player = QBCore.Functions.GetPlayer(src)
        local setamount = (StationBalance + amount)
        if Config.FuelDebug then print("Attempting to deposit 𝕍"..amount.." to Location #"..location.."'s Balance!") end
    ...
    RegisterNetEvent('cdn-fuel:station:server:updatefuelprice', function(fuelprice, location)
        local src = source
        if Config.FuelDebug then print('Attempting to update Location #'..location.."'s Fuel Price to a new price: 𝕍"..fuelprice) end
    ...
    RegisterNetEvent('cdn-fuel:stations:server:buyreserves', function(location, price, amount)
        ...
        if Config.FuelDebug then print("Attempting Sale Server Side for location: #"..location.." for Price: 𝕍"..price) end
        if ReserveBuyPossible and Player.Functions.RemoveMoney("bank", price, "Purchased"..amount.."L of Reserves for: "..Config.GasStations[location].label.." @ 𝕍"..Config.FuelReservesPrice.." / L!") then
